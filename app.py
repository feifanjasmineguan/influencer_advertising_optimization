...
def load_data():
    """
    Loads the graph and node features. 
    Cached so we don't reload 200MB files on every interaction.
    """
    # Paths
    # Note: Ensure these files are in a 'data' folder next to app.py
    nodes_path = "data/all_nodes_features.csv"
    edge_path = "data/higgs-social_network.edgelist"
    
    # Check if files exist
    if not os.path.exists(nodes_path) or not os.path.exists(edge_path):
        return None, None
    ...
    # 2. Load Graph (Check for compressed file if necessary)
    # If you zipped it to .gz, NetworkX handles it automatically
    G = nx.read_edgelist(
        edge_path,
